...
- **IdP group → role mapping — ✅ done.** `SSO_GROUP_ROLE_MAP` maps OIDC/SAML groups to EART roles (highest-privilege wins, `*` catch-all), synced on each login without clobbering manual roles. *Residual:* still only the 3 fixed role tiers (no custom roles/fine-grained permissions), and no scheduled reconciliation job for users who never log in again.
