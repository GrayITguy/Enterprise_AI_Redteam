...
- **SCIM 2.0 provisioning — ✅ done.** IdP-driven user provision/update/deprovision at `/scim/v2` (bearer-token auth), with a `is_active` deactivation flag enforced at login/SSO. *Residual:* IdP **group → role** mapping is minimal (SCIM `roles`/default only), and there's no automatic reconciliation job.
