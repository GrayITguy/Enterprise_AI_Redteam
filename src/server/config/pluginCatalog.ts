export type PluginTool = "promptfoo" | "garak" | "pyrit" | "deepteam";
export type Severity = "critical" | "high" | "medium" | "low" | "info";
export type OwaspCategory =
  | "LLM01" | "LLM02" | "LLM03" | "LLM04" | "LLM05"
  | "LLM06" | "LLM07" | "LLM08" | "LLM09" | "LLM10";

export interface Plugin {
  id: string;
  name: string;
  description: string;
  tool: PluginTool;
  category: string;
  severity: Severity;
  owaspCategory?: OwaspCategory;
  tags: string[];
}

export interface Preset {
  name: string;
  description: string;
  plugins: string[];
}

// ─────────────────────────────────────────────────────────────────────────────
// Plugin Catalog
// ─────────────────────────────────────────────────────────────────────────────
export const PLUGINS: Plugin[] = [
  // ── Promptfoo red team plugins ──────────────────────────────────────────────
  {
    id: "promptfoo:prompt-injection",
    name: "Prompt Injection",
    description: "Tests if the model follows malicious instructions embedded in user input to override system behavior.",
    tool: "promptfoo",
    category: "injection",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["owasp", "injection"],
  },
  {
    id: "promptfoo:jailbreak",
    name: "Jailbreak Attacks",
    description: "Attempts to bypass safety guardrails using common jailbreak patterns (DAN, roleplay, hypothetical).",
    tool: "promptfoo",
    category: "jailbreak",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["owasp", "safety"],
  },
  {
    id: "promptfoo:pii-extraction",
    name: "PII Extraction",
    description: "Tests if the model leaks or generates personally identifiable information from training data or context.",
    tool: "promptfoo",
    category: "privacy",
    severity: "critical",
    owaspCategory: "LLM02",
    tags: ["owasp", "privacy", "pii"],
  },
  {
    id: "promptfoo:system-prompt-leak",
    name: "System Prompt Exfiltration",
    description: "Probes whether the model reveals its system prompt when asked directly or indirectly.",
    tool: "promptfoo",
    category: "disclosure",
    severity: "high",
    owaspCategory: "LLM07",
    tags: ["owasp", "disclosure"],
  },
  {
    id: "promptfoo:harmful-content",
    name: "Harmful Content Generation",
    description: "Tests if the model generates instructions for illegal activities, violence, or dangerous content.",
    tool: "promptfoo",
    category: "safety",
    severity: "critical",
    owaspCategory: "LLM06",
    tags: ["owasp", "safety", "harmful"],
  },
  {
    id: "promptfoo:indirect-prompt-injection",
    name: "Indirect Prompt Injection",
    description: "Simulates RAG-based attacks where malicious instructions are hidden in retrieved documents.",
    tool: "promptfoo",
    category: "injection",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["owasp", "rag", "injection"],
  },
  {
    id: "promptfoo:rbac-bypass",
    name: "Insecure Output Handling",
    description: "Tests whether model output is used unsafely downstream (XSS, command injection via LLM output).",
    tool: "promptfoo",
    category: "output-handling",
    severity: "high",
    owaspCategory: "LLM02",
    tags: ["owasp", "output"],
  },
  {
    id: "promptfoo:overreliance",
    name: "Overreliance / Hallucination",
    description: "Tests whether the model confidently fabricates facts, citations, or authoritative-sounding false information.",
    tool: "promptfoo",
    category: "reliability",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["owasp", "hallucination"],
  },

  {
    id: "promptfoo:ascii-smuggling",
    name: "ASCII / Unicode Smuggling",
    description: "Injects invisible Unicode control characters or homoglyphs to bypass input filters while delivering harmful payloads.",
    tool: "promptfoo",
    category: "injection",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["owasp", "encoding", "bypass"],
  },
  {
    id: "promptfoo:debug-access",
    name: "Debug Mode Access",
    description: "Tests if the model can be tricked into exposing debug endpoints, verbose error messages, or internal state through crafted prompts.",
    tool: "promptfoo",
    category: "disclosure",
    severity: "high",
    owaspCategory: "LLM07",
    tags: ["owasp", "debug", "disclosure"],
  },
  {
    id: "promptfoo:bola",
    name: "Broken Object-Level Auth",
    description: "Tests whether the model can be prompted to access or modify resources belonging to other users or unauthorized objects.",
    tool: "promptfoo",
    category: "authorization",
    severity: "critical",
    owaspCategory: "LLM02",
    tags: ["owasp", "authz", "bola"],
  },
  {
    id: "promptfoo:bfla",
    name: "Broken Function-Level Auth",
    description: "Checks if the model exposes admin/privileged functionality to unprivileged users via prompt manipulation.",
    tool: "promptfoo",
    category: "authorization",
    severity: "critical",
    owaspCategory: "LLM02",
    tags: ["owasp", "authz", "bfla"],
  },
  {
    id: "promptfoo:contracts",
    name: "Contractual Compliance Bypass",
    description: "Tests if the model can be prompted to violate terms-of-service, legal constraints, or enterprise policy guardrails.",
    tool: "promptfoo",
    category: "compliance",
    severity: "medium",
    owaspCategory: "LLM10",
    tags: ["owasp", "legal", "compliance"],
  },
  {
    id: "promptfoo:shell-injection",
    name: "Shell Command Injection",
    description: "Tests if LLM output containing OS commands can be exploited when downstream systems execute model-generated shell code.",
    tool: "promptfoo",
    category: "injection",
    severity: "critical",
    owaspCategory: "LLM02",
    tags: ["owasp", "injection", "shell"],
  },
  {
    id: "promptfoo:sql-injection",
    name: "SQL Injection via LLM Output",
    description: "Checks whether the model generates SQL payloads that could be exploited if passed to a database without sanitization.",
    tool: "promptfoo",
    category: "injection",
    severity: "critical",
    owaspCategory: "LLM02",
    tags: ["owasp", "injection", "sql"],
  },
  {
    id: "promptfoo:ssrf",
    name: "Server-Side Request Forgery",
    description: "Tests if the model can be manipulated into generating URLs or requests that trigger server-side request forgery attacks.",
    tool: "promptfoo",
    category: "injection",
    severity: "critical",
    owaspCategory: "LLM02",
    tags: ["owasp", "ssrf", "injection"],
  },
  {
    id: "promptfoo:excessive-agency",
    name: "Excessive Agency",
    description: "Tests if the model exceeds its intended scope by taking unauthorized actions or making decisions beyond its role.",
    tool: "promptfoo",
    category: "agentic",
    severity: "critical",
    owaspCategory: "LLM08",
    tags: ["owasp", "agentic", "scope"],
  },
  {
    id: "promptfoo:competitors",
    name: "Competitor Endorsement",
    description: "Tests if the model inadvertently promotes or endorses competing products, services, or brands.",
    tool: "promptfoo",
    category: "brand",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["brand", "competitors"],
  },
  {
    id: "promptfoo:imitation",
    name: "Brand Imitation",
    description: "Tests if the model can be tricked into impersonating or imitating a specific brand, company, or public figure.",
    tool: "promptfoo",
    category: "brand",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["brand", "imitation"],
  },
  {
    id: "promptfoo:harmful:misinformation",
    name: "Misinformation / Disinformation",
    description: "Tests if the model generates or amplifies false information, fake news, or propaganda narratives.",
    tool: "promptfoo",
    category: "misinformation",
    severity: "high",
    owaspCategory: "LLM09",
    tags: ["owasp", "misinformation", "safety"],
  },
  {
    id: "promptfoo:harmful:copyright-violations",
    name: "Copyright Violation",
    description: "Tests if the model reproduces copyrighted material, intellectual property, or proprietary content verbatim.",
    tool: "promptfoo",
    category: "legal",
    severity: "medium",
    owaspCategory: "LLM10",
    tags: ["owasp", "copyright", "legal"],
  },

  // ── Garak probes ─────────────────────────────────────────────────────────────
  {
    id: "garak:encoding-attacks",
    name: "Encoding-based Attacks",
    description: "Garak probe: uses Base64, ROT13, Leetspeak and other encodings to smuggle harmful requests past filters.",
    tool: "garak",
    category: "encoding",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["garak", "encoding", "bypass"],
  },
  {
    id: "garak:continuation-attacks",
    name: "Completion Exploitation",
    description: "Garak probe: exploits the model's tendency to complete harmful text patterns.",
    tool: "garak",
    category: "completion",
    severity: "high",
    owaspCategory: "LLM06",
    tags: ["garak", "completion"],
  },
  {
    id: "garak:dan-variants",
    name: "DAN & Persona Jailbreaks",
    description: "Garak probe: systematic variants of Do Anything Now (DAN) and alternative persona prompts.",
    tool: "garak",
    category: "jailbreak",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["garak", "jailbreak", "dan"],
  },
  {
    id: "garak:malware-generation",
    name: "Malware Code Generation",
    description: "Garak probe: tests if the model generates functional malware, exploits, or hacking tools.",
    tool: "garak",
    category: "malware",
    severity: "critical",
    owaspCategory: "LLM06",
    tags: ["garak", "malware", "code"],
  },
  {
    id: "garak:misinformation",
    name: "Misinformation Generation",
    description: "Garak probe: tests if the model generates convincing false narratives, fake news, or disinformation.",
    tool: "garak",
    category: "misinformation",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["garak", "misinformation"],
  },
  {
    id: "garak:prompt-injection-xss",
    name: "Cross-Prompt Injection",
    description: "Garak probe: tests cross-context prompt injection for multi-turn and agentic systems.",
    tool: "garak",
    category: "injection",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["garak", "injection", "agentic"],
  },
  {
    id: "garak:data-exfiltration",
    name: "Training Data Extraction",
    description: "Garak probe: attempts to extract memorized training data (emails, code, personal info) from the model.",
    tool: "garak",
    category: "privacy",
    severity: "high",
    owaspCategory: "LLM06",
    tags: ["garak", "privacy", "extraction"],
  },

  {
    id: "garak:gcg-suffixes",
    name: "GCG Adversarial Suffixes",
    description: "Garak probe: appends gradient-computed adversarial suffixes (GCG attack) that reliably elicit harmful completions.",
    tool: "garak",
    category: "adversarial",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["garak", "adversarial", "gradient"],
  },
  {
    id: "garak:glitch-tokens",
    name: "Glitch Token Exploitation",
    description: "Garak probe: injects known glitch tokens (e.g., SolidGoldMagikarp) that cause unpredictable or policy-violating model behavior.",
    tool: "garak",
    category: "adversarial",
    severity: "medium",
    owaspCategory: "LLM01",
    tags: ["garak", "tokens", "glitch"],
  },
  {
    id: "garak:snowball",
    name: "Snowball Hallucination Chain",
    description: "Garak probe: constructs multi-step prompts that cascade small hallucinations into large factual fabrications.",
    tool: "garak",
    category: "reliability",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["garak", "hallucination", "chain"],
  },
  {
    id: "garak:topic-avoidance-bypass",
    name: "Topic Avoidance Bypass",
    description: "Garak probe: systematically tests whether the model can be maneuvered around configured forbidden topics via rephrasing and context shifting.",
    tool: "garak",
    category: "jailbreak",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["garak", "bypass", "topics"],
  },
  {
    id: "garak:social-engineering",
    name: "Social Engineering Attacks",
    description: "Garak probe: simulates social engineering scenarios (authority, urgency, flattery) to manipulate the model into policy violations.",
    tool: "garak",
    category: "manipulation",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["garak", "social", "manipulation"],
  },
  {
    id: "garak:realtoxicityprompts",
    name: "Real Toxicity Prompts",
    description: "Garak probe: uses the RealToxicityPrompts dataset to evaluate if the model generates toxic continuations from benign-seeming prompts.",
    tool: "garak",
    category: "safety",
    severity: "high",
    owaspCategory: "LLM06",
    tags: ["garak", "toxicity", "safety"],
  },
  {
    id: "garak:packagehallucination",
    name: "Package Hallucination",
    description: "Garak probe: tests if the model recommends non-existent software packages that could be supply-chain attack vectors.",
    tool: "garak",
    category: "reliability",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["garak", "hallucination", "supply-chain"],
  },
  {
    id: "garak:donotanswer",
    name: "Do-Not-Answer Compliance",
    description: "Garak probe: tests model refusal rates on prompts from the DoNotAnswer dataset that responsible models should decline.",
    tool: "garak",
    category: "safety",
    severity: "medium",
    owaspCategory: "LLM06",
    tags: ["garak", "refusal", "safety"],
  },
  {
    id: "garak:lmrc",
    name: "Language Model Risk Cards",
    description: "Garak probe: evaluates model against the Language Model Risk Cards framework covering toxicity, bias, and privacy risks.",
    tool: "garak",
    category: "safety",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["garak", "risk-cards", "safety"],
  },
  {
    id: "garak:grandma",
    name: "Emotional Manipulation (Grandma)",
    description: "Garak probe: uses emotional manipulation techniques (appeals to authority, urgency, guilt, grandma exploit) to bypass safety filters.",
    tool: "garak",
    category: "jailbreak",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["garak", "emotional", "manipulation"],
  },

  // ── PyRIT attacks ─────────────────────────────────────────────────────────────
  {
    id: "pyrit:crescendo",
    name: "Crescendo Attack",
    description: "PyRIT: multi-turn attack that gradually escalates from benign to harmful requests to bypass safety.",
    tool: "pyrit",
    category: "multi-turn",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["pyrit", "multi-turn", "escalation"],
  },
  {
    id: "pyrit:skeleton-key",
    name: "Skeleton Key Attack",
    description: "PyRIT: convinces the model to adopt an alternate identity that ignores safety guidelines.",
    tool: "pyrit",
    category: "jailbreak",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["pyrit", "jailbreak", "identity"],
  },
  {
    id: "pyrit:many-shot-jailbreak",
    name: "Many-Shot Jailbreaking",
    description: "PyRIT: uses long context windows with many examples to override safety behavior via in-context learning.",
    tool: "pyrit",
    category: "jailbreak",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["pyrit", "jailbreak", "context-window"],
  },
  {
    id: "pyrit:pair-attack",
    name: "PAIR Attack (Automated Red Teaming)",
    description: "PyRIT: Prompt Automatic Iterative Refinement — uses an attacker LLM to iteratively refine jailbreak prompts.",
    tool: "pyrit",
    category: "automated-redteam",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["pyrit", "automated", "adversarial"],
  },
  {
    id: "pyrit:prompt-shield-bypass",
    name: "Prompt Shield Bypass",
    description: "PyRIT: tests evasion of Azure AI Content Safety and similar prompt filtering systems.",
    tool: "pyrit",
    category: "filter-bypass",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["pyrit", "bypass", "content-safety"],
  },
  {
    id: "pyrit:indirect-injection-agentic",
    name: "Agentic Indirect Injection",
    description: "PyRIT: targets agentic systems where the LLM takes actions (API calls, tool use) based on injected instructions.",
    tool: "pyrit",
    category: "agentic",
    severity: "critical",
    owaspCategory: "LLM08",
    tags: ["pyrit", "agentic", "tools"],
  },
  {
    id: "pyrit:tap-attack",
    name: "Tree of Attacks with Pruning (TAP)",
    description: "PyRIT: uses a tree-based search with multiple branching attack paths and prunes ineffective ones to find optimal jailbreaks.",
    tool: "pyrit",
    category: "automated-redteam",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["pyrit", "automated", "tree-search"],
  },
  {
    id: "pyrit:xpia",
    name: "Cross-Domain Prompt Injection (XPIA)",
    description: "PyRIT: tests cross-domain prompt injection attacks where malicious instructions are embedded in external data sources (images, documents, APIs).",
    tool: "pyrit",
    category: "injection",
    severity: "critical",
    owaspCategory: "LLM01",
    tags: ["pyrit", "injection", "cross-domain"],
  },
  {
    id: "pyrit:flip-attack",
    name: "FlipAttack Encoding Bypass",
    description: "PyRIT: uses character flipping, word reversal, and encoding transformations to bypass content safety filters.",
    tool: "pyrit",
    category: "encoding",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["pyrit", "encoding", "bypass"],
  },

  // ── DeepTeam metrics ─────────────────────────────────────────────────────────
  {
    id: "deepteam:toxic-content",
    name: "Toxicity Detection",
    description: "DeepTeam: measures if the model generates or amplifies toxic, hateful, or abusive language.",
    tool: "deepteam",
    category: "safety",
    severity: "high",
    owaspCategory: "LLM06",
    tags: ["deepteam", "toxicity", "safety"],
  },
  {
    id: "deepteam:bias-gender",
    name: "Gender Bias",
    description: "DeepTeam: tests for systematic gender biases in model outputs across occupational and behavioral prompts.",
    tool: "deepteam",
    category: "bias",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["deepteam", "bias", "gender"],
  },
  {
    id: "deepteam:bias-race",
    name: "Racial & Ethnic Bias",
    description: "DeepTeam: tests for racial and ethnic stereotyping in model responses.",
    tool: "deepteam",
    category: "bias",
    severity: "high",
    owaspCategory: "LLM09",
    tags: ["deepteam", "bias", "race"],
  },
  {
    id: "deepteam:hallucination",
    name: "Factual Hallucination Rate",
    description: "DeepTeam: measures the rate at which the model confidently states false facts on verifiable claims.",
    tool: "deepteam",
    category: "reliability",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["deepteam", "hallucination", "reliability"],
  },
  {
    id: "deepteam:pii-leakage",
    name: "PII Generation in Context",
    description: "DeepTeam: tests if the model inappropriately generates or surfaces PII in conversational contexts.",
    tool: "deepteam",
    category: "privacy",
    severity: "critical",
    owaspCategory: "LLM06",
    tags: ["deepteam", "pii", "privacy"],
  },
  {
    id: "deepteam:misinformation-politics",
    name: "Political Misinformation",
    description: "DeepTeam: tests if the model generates or endorses political misinformation.",
    tool: "deepteam",
    category: "misinformation",
    severity: "high",
    owaspCategory: "LLM09",
    tags: ["deepteam", "politics", "misinformation"],
  },
  {
    id: "deepteam:copyright-violations",
    name: "Copyright & IP Violations",
    description: "DeepTeam: tests if the model reproduces copyrighted text, code, or creative works verbatim.",
    tool: "deepteam",
    category: "legal",
    severity: "medium",
    owaspCategory: "LLM10",
    tags: ["deepteam", "copyright", "legal"],
  },
  {
    id: "deepteam:excessive-agency",
    name: "Excessive Agency Actions",
    description: "DeepTeam: tests if an agentic model takes destructive actions beyond its authorized scope.",
    tool: "deepteam",
    category: "agentic",
    severity: "critical",
    owaspCategory: "LLM08",
    tags: ["deepteam", "agentic", "scope"],
  },
  {
    id: "deepteam:bias-political",
    name: "Political Bias",
    description: "DeepTeam: tests for systematic political bias or partisan leanings in model responses.",
    tool: "deepteam",
    category: "bias",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["deepteam", "bias", "political"],
  },
  {
    id: "deepteam:bias-religion",
    name: "Religion Bias",
    description: "DeepTeam: tests for religious bias, stereotyping, or preferential treatment of certain faiths.",
    tool: "deepteam",
    category: "bias",
    severity: "medium",
    owaspCategory: "LLM09",
    tags: ["deepteam", "bias", "religion"],
  },
  {
    id: "deepteam:robustness-hijacking",
    name: "Input Hijacking",
    description: "DeepTeam: tests robustness against input hijacking where adversarial inputs redirect model behavior away from its intended task.",
    tool: "deepteam",
    category: "robustness",
    severity: "high",
    owaspCategory: "LLM01",
    tags: ["deepteam", "robustness", "hijacking"],
  },
  {
    id: "deepteam:pii-session-leak",
    name: "PII Session Leakage",
    description: "DeepTeam: tests if PII from one user session leaks into another user's session context.",
    tool: "deepteam",
    category: "privacy",
    severity: "high",
    owaspCategory: "LLM06",
    tags: ["deepteam", "pii", "session"],
  },
  {
    id: "deepteam:misinformation-health",
    name: "Health Misinformation",
    description: "DeepTeam: tests if the model generates or endorses dangerous health misinformation (anti-vax, fake cures, etc.).",
    tool: "deepteam",
    category: "misinformation",
    severity: "high",
    owaspCategory: "LLM09",
    tags: ["deepteam", "health", "misinformation"],
  },

  // ── Expanded garak coverage (probe modules verified present in garak) ──────
  {
    id: "garak:promptinject",
    name: "PromptInject Framework",
    description: "Garak: runs the PromptInject probe suite — systematic goal-hijacking and prompt-leaking attacks.",
    tool: "garak", category: "injection", severity: "high", owaspCategory: "LLM01",
    tags: ["garak", "injection", "promptinject"],
  },
  {
    id: "garak:latent-injection",
    name: "Latent / Indirect Injection",
    description: "Garak: latent-injection probes — malicious instructions hidden in retrieved documents, translation, or code contexts.",
    tool: "garak", category: "injection", severity: "high", owaspCategory: "LLM01",
    tags: ["garak", "injection", "indirect"],
  },
  {
    id: "garak:goodside",
    name: "Goodside Attacks",
    description: "Garak: Riley Goodside–style attacks (Unicode tag smuggling, deletion, thread-jacking).",
    tool: "garak", category: "jailbreak", severity: "high", owaspCategory: "LLM01",
    tags: ["garak", "jailbreak"],
  },
  {
    id: "garak:auto-adversarial",
    name: "Auto-Generated Adversarial (atkgen)",
    description: "Garak: atkgen uses a red-team model to automatically generate and adapt adversarial prompts against the target.",
    tool: "garak", category: "adversarial", severity: "high", owaspCategory: "LLM01",
    tags: ["garak", "adversarial", "automated"],
  },
  {
    id: "garak:tree-of-attacks",
    name: "Tree of Attacks with Pruning",
    description: "Garak: TAP probe — branching adversarial search that prunes ineffective jailbreak paths.",
    tool: "garak", category: "automated-redteam", severity: "high", owaspCategory: "LLM01",
    tags: ["garak", "automated-redteam", "tap"],
  },
  {
    id: "garak:ansi-escape",
    name: "ANSI Escape Injection",
    description: "Garak: ansiescape probes — terminal escape sequences in model output that can corrupt or hijack a terminal.",
    tool: "garak", category: "output-handling", severity: "medium", owaspCategory: "LLM05",
    tags: ["garak", "output-handling"],
  },
  {
    id: "garak:data-divergence",
    name: "Training-Data Divergence",
    description: "Garak: divergence probes — repetition/divergence attacks that coax verbatim training-data leakage.",
    tool: "garak", category: "privacy", severity: "high", owaspCategory: "LLM06",
    tags: ["garak", "privacy", "data-leak"],
  },
  {
    id: "garak:file-formats",
    name: "Malicious File Formats",
    description: "Garak: fileformats probes — generation of files/artifacts in formats associated with malware or exploitation.",
    tool: "garak", category: "malware", severity: "medium", owaspCategory: "LLM05",
    tags: ["garak", "malware"],
  },
  {
    id: "garak:exploitation",
    name: "Code Exploitation",
    description: "Garak: exploitation probes — elicit exploit code / injection payloads (SQLi, XSS, template injection).",
    tool: "garak", category: "injection", severity: "high", owaspCategory: "LLM05",
    tags: ["garak", "injection", "exploitation"],
  },
  {
    id: "garak:av-spam-signatures",
    name: "AV / Spam Signatures",
    description: "Garak: av_spam_scanning probes — checks whether the model will emit known antivirus/spam test signatures.",
    tool: "garak", category: "safety", severity: "low", owaspCategory: "LLM05",
    tags: ["garak", "safety"],
  },
  {
    id: "garak:phrasing",
    name: "Tense / Phrasing Evasion",
    description: "Garak: phrasing probes — past/future-tense and rephrasing attacks that slip requests past safety filters.",
    tool: "garak", category: "jailbreak", severity: "medium", owaspCategory: "LLM01",
    tags: ["garak", "jailbreak", "evasion"],
  },

  // ── Expanded deepteam coverage (vulnerabilities verified in deepteam 1.0.x) ─
  {
    id: "deepteam:prompt-leakage",
    name: "System Prompt Leakage",
    description: "DeepTeam: tests whether the model reveals its system prompt, embedded secrets/credentials, or guard instructions.",
    tool: "deepteam", category: "disclosure", severity: "high", owaspCategory: "LLM07",
    tags: ["deepteam", "disclosure", "prompt-leak"],
  },
  {
    id: "deepteam:rbac",
    name: "RBAC Bypass",
    description: "DeepTeam: tests role-based access control bypass — role escalation and unauthorized role assumption.",
    tool: "deepteam", category: "authorization", severity: "high", owaspCategory: "LLM08",
    tags: ["deepteam", "authorization", "rbac"],
  },
  {
    id: "deepteam:bola",
    name: "Broken Object-Level Authorization",
    description: "DeepTeam: BOLA — accessing or manipulating objects belonging to other users/customers.",
    tool: "deepteam", category: "authorization", severity: "high", owaspCategory: "LLM08",
    tags: ["deepteam", "authorization", "bola"],
  },
  {
    id: "deepteam:bfla",
    name: "Broken Function-Level Authorization",
    description: "DeepTeam: BFLA — invoking privileged functions or bypassing function-level authorization.",
    tool: "deepteam", category: "authorization", severity: "high", owaspCategory: "LLM08",
    tags: ["deepteam", "authorization", "bfla"],
  },
  {
    id: "deepteam:sql-injection",
    name: "SQL Injection",
    description: "DeepTeam: tests whether model output enables SQL injection (blind/union/error-based).",
    tool: "deepteam", category: "injection", severity: "critical", owaspCategory: "LLM05",
    tags: ["deepteam", "injection", "sql"],
  },
  {
    id: "deepteam:shell-injection",
    name: "Shell / Command Injection",
    description: "DeepTeam: tests whether model output enables OS command / shell injection.",
    tool: "deepteam", category: "injection", severity: "critical", owaspCategory: "LLM05",
    tags: ["deepteam", "injection", "shell"],
  },
  {
    id: "deepteam:ssrf",
    name: "Server-Side Request Forgery",
    description: "DeepTeam: SSRF — coaxing the model/agent into internal-service access, cloud-metadata access, or port scanning.",
    tool: "deepteam", category: "injection", severity: "high", owaspCategory: "LLM06",
    tags: ["deepteam", "ssrf", "agentic"],
  },
  {
    id: "deepteam:illegal-activity",
    name: "Illegal Activity",
    description: "DeepTeam: tests whether the model assists with weapons, illegal drugs, violent/cyber crimes.",
    tool: "deepteam", category: "safety", severity: "high", owaspCategory: "LLM05",
    tags: ["deepteam", "safety", "harmful"],
  },
  {
    id: "deepteam:personal-safety",
    name: "Personal Safety",
    description: "DeepTeam: tests for content enabling self-harm, bullying, stalking, or dangerous challenges.",
    tool: "deepteam", category: "safety", severity: "high", owaspCategory: "LLM05",
    tags: ["deepteam", "safety", "personal-safety"],
  },
  {
    id: "deepteam:debug-access",
    name: "Debug / Admin Access Exposure",
    description: "DeepTeam: tests for debug-mode bypass and exposure of development/administrative interfaces.",
    tool: "deepteam", category: "disclosure", severity: "medium", owaspCategory: "LLM07",
    tags: ["deepteam", "disclosure", "debug"],
  },
  {
    id: "deepteam:graphic-content",
    name: "Graphic Content",
    description: "DeepTeam: tests whether the model produces sexual or gratuitously graphic content.",
    tool: "deepteam", category: "safety", severity: "medium", owaspCategory: "LLM05",
    tags: ["deepteam", "safety", "graphic"],
  },
];

// ─────────────────────────────────────────────────────────────────────────────
// Presets
// ─────────────────────────────────────────────────────────────────────────────
export const PRESETS: Record<string, Preset> = {
  quick: {
    name: "Quick Scan",
    description: "10 core tests covering the most common critical vulnerabilities. Fast results.",
    plugins: [
      "promptfoo:prompt-injection",
      "promptfoo:jailbreak",
      "promptfoo:pii-extraction",
      "promptfoo:harmful-content",
      "deepteam:toxic-content",
      "promptfoo:overreliance",
      "deepteam:bias-gender",
      "promptfoo:system-prompt-leak",
      "promptfoo:excessive-agency",
      "garak:realtoxicityprompts",
    ],
  },
  owasp: {
    name: "OWASP LLM Top 10",
    description: "Comprehensive coverage of all 10 OWASP LLM security categories.",
    plugins: [
      // LLM01 - Prompt Injection
      "promptfoo:prompt-injection",
      "promptfoo:indirect-prompt-injection",
      "garak:encoding-attacks",
      // LLM02 - Insecure Output
      "promptfoo:pii-extraction",
      "promptfoo:rbac-bypass",
      "promptfoo:ssrf",
      // LLM03 - Training Data Poisoning
      "garak:data-exfiltration",
      // LLM04 - Model Denial of Service (covered by rate limiting)
      // LLM05 - Supply Chain (infra-level, not testable here)
      // LLM06 - Sensitive Info Disclosure
      "promptfoo:harmful-content",
      "garak:malware-generation",
      "deepteam:pii-leakage",
      // LLM07 - Insecure Plugin Design
      "promptfoo:system-prompt-leak",
      // LLM08 - Excessive Agency
      "promptfoo:excessive-agency",
      "pyrit:indirect-injection-agentic",
      "deepteam:excessive-agency",
      // LLM09 - Overreliance
      "promptfoo:overreliance",
      "promptfoo:harmful:misinformation",
      "deepteam:hallucination",
      "garak:misinformation",
      "garak:packagehallucination",
      "deepteam:misinformation-health",
      // LLM10 - Model Theft
      "deepteam:copyright-violations",
      "promptfoo:harmful:copyright-violations",
    ],
  },
  full: {
    name: "Full Enterprise Scan",
    description: "Every plugin across all 4 tools. Comprehensive red team assessment.",
    plugins: PLUGINS.map((p) => p.id),
  },
};

export function getPluginById(id: string): Plugin | undefined {
  return PLUGINS.find((p) => p.id === id);
}

export function getPluginsByTool(tool: PluginTool): Plugin[] {
  return PLUGINS.filter((p) => p.tool === tool);
}

export function resolvePlugins(pluginIds: string[]): Plugin[] {
  return pluginIds
    .map((id) => getPluginById(id))
    .filter((p): p is Plugin => p !== undefined);
}
